...
class TablePlotter:
    ...
    def _get_cells(self, left, right, vertical):
        ...
        if vertical:
            ...
            vcells = max(
                sum(self._shape(l)[0] for l in left), self._shape(right)[0]
            )
            hcells = (
                max(self._shape(l)[1] for l in left) + self._shape(right)[1]
            )
        else:
            vcells = max(
                [self._shape(l)[0] for l in left] + [self._shape(right)[0]]
            )
            hcells = sum(
                [self._shape(l)[1] for l in left] + [self._shape(right)[1]]
            )
    ...
    def plot(self, left, right, labels=None, vertical=True):
        ...
        if vertical:
            ...
            for i, (l, label) in enumerate(zip(left, labels)):
                ...
                self._make_table(
                    ax, l, title=label, height=1.0 / max_left_rows
                )
            # right
    ...
    def _insert_index(self, data):
        ...
        if idx_nlevels == 1:
            ...
            for i in range(idx_nlevels):
                data.insert(
                    i, "Index{0}".format(i), data.index._get_level_values(i)
                )
        ...
        if col_nlevels > 1:
            ...
            values = [
                data.columns._get_level_values(i).values
...
if __name__ == "__main__":
    ...
    p.plot(
        [df1, df2], pd.concat([df1, df2]), labels=["df1", "df2"], vertical=True
    )
    plt.show()
    ...
    df3 = pd.DataFrame(
        {"v1": [1, 2, 3, 4, 5, 6], "v2": [5, 6, 7, 8, 9, 10]}, index=idx
    )
    df3.columns = col
