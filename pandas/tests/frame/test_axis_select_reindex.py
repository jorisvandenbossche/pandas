...
class TestDataFrameSelectReindex(TestData):
    ...
    def test_reindex_fill_value(self):
        ...
        with tm.assert_produces_warning(FutureWarning):
            result = df.reindex_axis(range(15), fill_value=0.0, axis=0)
        expected = df.reindex(range(15)).fillna(0)
        assert_frame_equal(result, expected)

        with tm.assert_produces_warning(FutureWarning):
            result = df.reindex_axis(range(5), fill_value=0.0, axis=1)
    ...
    def test_align(self):
        ...
        af, bf = self.mixed_frame.align(
            self.mixed_frame, join="inner", axis=1, method="pad"
        )
        ...
        # mixed floats/ints
        af, bf = self.mixed_float.align(
            other.iloc[:, 0], join="inner", axis=1, method=None, fill_value=0
        )
        tm.assert_index_equal(bf.index, Index([]))

        af, bf = self.mixed_int.align(
            other.iloc[:, 0], join="inner", axis=1, method=None, fill_value=0
        )
        tm.assert_index_equal(bf.index, Index([]))
        ...
        expected = DataFrame(
            expected, index=self.frame.index, columns=self.frame.columns
        )
