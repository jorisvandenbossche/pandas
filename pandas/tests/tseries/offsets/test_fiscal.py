...
class TestFY5253NearestEndMonth(Base):
    def test_get_year_end(self):
        ...
        offset_n = FY5253(
            weekday=WeekDay.TUE, startingMonth=12, variation="nearest"
        )
        assert offset_n.get_year_end(datetime(2012, 1, 1)) == datetime(
            2013, 1, 1
        )
        assert offset_n.get_year_end(datetime(2012, 1, 10)) == datetime(
            2013, 1, 1
        )

        assert offset_n.get_year_end(datetime(2013, 1, 1)) == datetime(
            2013, 12, 31
        )
        assert offset_n.get_year_end(datetime(2013, 1, 2)) == datetime(
            2013, 12, 31
        )
        assert offset_n.get_year_end(datetime(2013, 1, 3)) == datetime(
            2013, 12, 31
        )
        assert offset_n.get_year_end(datetime(2013, 1, 10)) == datetime(
            2013, 12, 31
        )
...
class TestFY5253NearestEndMonthQuarter(Base):
    ...
    def test_offset(self):
        ...
        assert_offset_equal(
            offset, datetime(2012, 5, 31), datetime(2012, 8, 30)
        )
        assert_offset_equal(
            offset, datetime(2012, 5, 30), datetime(2012, 5, 31)
        )
        ...
        assert_offset_equal(
            offset2, datetime(2013, 1, 15), datetime(2013, 3, 30)
        )
...
def test_fy5253_last_onoffset():
    ...
    ts = Timestamp(
        "1984-05-28 06:29:43.955911354+0200", tz="Europe/San_Marino"
    )
